...
//韻ふみシステム
public class RapJudger_buckup2 : MonoBehaviour
{
    public string RapSentence;

    const float RhymeImportanceScale=3f;//累乗される
    const float ConsonantImportanceScale=1.6f;//0か1かこれ
    
    List<string> ls00= new List<string>();
    ...
    void Start()
    {
        ...
        //JudgeRap("おじいさんの言いがかりおばあさんは聞く係り");
    ...
    //ラップ判定
    void JudgeRap(string st00)
    {
        ls00=SentenceToHiraganas(st00);
        ...
        {
            for(var j = 0; j < ls00.Count; j++)
            {
                if(Mathf.Abs(i-j)<=4)//距離が4以内の単語のみ韻を踏んでるか判断する
                {
                    Debug.Log($"{i}{ls00[i]}{j}{ls00[j]}");
                    Debug.Log("韻の数は"+JudgeMaxVowelCombo(ls00[i],ls00[j]));
                }
            }
        }
    }
    //文からカタカナ形態素
    List<string> SentenceToHiraganas(string st01)
    {
        List<string> ls01= new List<string>();
        ...
        using (var tagger = MeCabIpaDicTagger.Create(dicDir))
        {
            ...
            {
                //Debug.Log($"{item.Reading}");
                ls01.Add(item.Reading);
            }
        }
        return(ls01);
    }
